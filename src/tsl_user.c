...
extern uint16_t swipe_delay_value;

uint16_t swipe_delay = 0;
bool swipe_ST = FALSE;
bool swipe_0 = FALSE;
bool swipe_1 = FALSE;
bool swipe_2 = FALSE;
bool swipe_END = FALSE;
bool LedON = FALSE;
//==============================================================================
// Channels
//==============================================================================
...
TSL_Status_enum_T TSL_user_Action(void)
{
  ...
  if(idx_bank == TSLPRM_TOTAL_BANKS)
  {
    ...
    // ECS every 100ms
    if (TSL_tim_CheckDelay_ms(5, &ECS_last_tick) == TSL_STATUS_OK)
    {
//      LED_BLUE_TOGGLE;
...
void ProcessSensorsSwipe(void)
{
  uint8_t Message[6];
  uint8_t Pos=0;

  if (swipe_END == TRUE && LedON == FALSE)
  {
	  swipe_0 = FALSE;
	  swipe_1 = FALSE;
	  swipe_2 = FALSE;
    ...
    if (process_sensor)
    {
    	Pos=MyLinRots[0].p_Data->Position;
        /* Prepare Message to display*/
        Message[0] = ' ';
        Message[1] = '0';
        Message[2] = '0';
		...
		{
			Message[4] = 255;
			if(swipe_2)swipe_END = TRUE;
			else swipe_END = FALSE;
		}
		else if( Pos > 80 )
		{
			Message[3] = 255;
			if(swipe_1)swipe_2 = TRUE;
			else swipe_2 = FALSE;
		}
		else if( Pos > 5 )
		{
	  ...
	  if(swipe_END)Message[4] = 255;
	  if(swipe_2)Message[3] = 255;
	  if(swipe_1)Message[2] = 255;
	  if(swipe_0)Message[1] = 255;
